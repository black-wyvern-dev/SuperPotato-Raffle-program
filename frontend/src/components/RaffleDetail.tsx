...
import { BackIcon, DownIcon, SolanaIcon, TwitterIcon, UserIcon, VerifiedIcon } from "./svgIcons";
...
export default function RaffleDetail(props: {
    ...
    const [isRaffler, setIsRaffler] = useState(false);
    const [isRevealed, setIsRevealed] = useState(false);
    const [isWinner, setIsWinner] = useState(false);
    const [isClaimed, setIsClaimed] = useState(false);
    const [isWidraw, setIsWidraw] = useState(false);
    ...
    const getRaffleDetail = async () => {
        ...
        if (raffleDetail.nftMint === "") return;
        ...
        console.log(raffleData, "====> raffle data s")
        ...
        if (raffleData.creator.toBase58() === wallet.publicKey?.toBase58()) setIsRaffler(true);
        if (raffleData.winner.toBase58() === wallet.publicKey?.toBase58()) setIsWinner(true);
        if (raffleData.claimed.toNumber() === 1) setIsClaimed(true);
        if (raffleData.claimed.toNumber() === 2) setIsRevealed(true);
        if (raffleData.claimed.toNumber() === 3) setIsWidraw(true);
                    ...
                    <>
                        <div className="content-header">
                            ...
                            <div className="detail-name">
                                <label>Collection <VerifiedIcon color="#0265DA" /></label>
                                <p>{raffleDetail.name}</p>
                            </div>
                        </div>
                        <div className="nft-detail">
                            ...
                            <div className="nft-detail-content">
                                        ...
                                        <>
                                            ...
                                            {new Date() < new Date(raffleDetail.endTimestamp * 1000) ?
                                                ...
                                                (
                                                    isRaffler ?
                                                        (!isWidraw ?
                                                            <button className={`btn-buy btn-full`} onClick={() => onWidrawNft()}>
                                                                Widraw NFT
                                                            </button> : <span></span>
                                                        )
                                                        :
                                                        (
                                                            raffleDetail.count !== 0 ?
                                                                (
                                                                    !isRevealed ?
                                                                        <button className={`btn-buy btn-full`} onClick={() => onRevealWinner()}>
                                                                            Reveal Winner
                                                                        </button>
                                                                        :
                                                                        (
                                                                            isWinner ?
                                                                                (
                                                                                    !isClaimed ?
                                                                                        <button className={`btn-buy btn-full`} onClick={() => onClaimNft()}>
                                                                                            Claim NFT
                                                                                        </button>
                                                                                        :
                                                                                        <span></span>
                                ...
                                <div className="detail-row">
                                    ...
                                    <p>{raffleDetail.ticketPriceSol} <SolanaIcon /></p>
                                </div>
                                <div className="detail-row">
                                    <label>Entries Left</label>
                                    <p>{raffleDetail.count} / {raffleDetail.maxEntrants}</p>
                                </div>
                                <div className="detail-row">
                                    <label>
                                        {new Date() > new Date(raffleDetail.endTimestamp * 1000) ?
                                    ...
                                    <p>
                                        <EndTimeCountdown endTime={moment(raffleDetail.endTimestamp * 1000).format("yyyy-MM-DD HH:mm")} endAction={() => getRaffleDetail()} />
                                    </p>
                                </div>
